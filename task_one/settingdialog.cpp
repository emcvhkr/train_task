...
void SettingDialog::initTab4()
{
    ...
    language_info = new QLabel();
    language_change = new QComboBox();
    language_change->addItem(tr("English"));
    QString cn = QString::fromLocal8Bit("简体中文");
    char *cn_t;
    std::string cn_tt = cn.toStdString();
    cn_t = (char*)cn_tt.c_str();
    language_change->addItem(tr(cn_t));
    ...
    group_layout->setSpacing(0);
    group_layout->addWidget(language_info);
    group_layout->addWidget(language_change);
    group_layout->setContentsMargins(30, 0, 0, 0);
...
void SettingDialog::initBottom()
{
    ...
    connect(ok_button,SIGNAL(clicked()),this,SLOT(adopt_settings()));
...
void SettingDialog::translateLanguage()
{
    ...
    language_info->setText(tr("Language"));
...
void SettingDialog::adopt_settings()
{
    qDebug()<<"INDEX"<<language_change->currentIndex();
    if(ZH == language_change->currentIndex())
    {
        qDebug()<<"ZH";
        m_translater.load(":/lang/zh");
        qDebug()<<"IS_TRUE?"<<m_translater.load(":/lang/zh");
        qApp->installTranslator(&m_translater);

    }
    if(EN == language_change->currentIndex())
    {
        qDebug()<<"EN";
        m_translater.load(":/lang/en");
