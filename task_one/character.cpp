    ...
    :QWidget(parent)
{
    ...
    background_label->setPixmap(QPixmap(":/img/newCharacter/bg_bottom_p"));
    ...
    for(int i = 0; i < WINDOW_PAGE_COUNT; i++)
    {
        painter.drawImage(QRect(WINDOW_WIDTH*i, 0, WINDOW_WIDTH, WINDOW_HEIGHT),
            QImage(QString(":/img/newCharacter/desktop_%1_p").arg(i)));
    }
    ...
    close_button = new NPushButton(this);
    ...
    for(int i = 0; i < WINDOW_BUTTON_COUNT; i++)
    {
        ...
        label->setPixmap(QPixmap(QString(":/img/newCharacter/btn_%1_p").arg(i)));
